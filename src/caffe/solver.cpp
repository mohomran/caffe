...
void AdaDeltaSolver<Dtype>::PreSolve() {
  ...
  for (int i = 0; i < net_params.size(); ++i) {
    ...
    this->update_.push_back(shared_ptr<Blob<Dtype> >(new Blob<Dtype>(
        net_param->num(), net_param->channels(), net_param->height(),
        net_param->width())));
    this->update_history_.push_back(shared_ptr<Blob<Dtype> >(new Blob<Dtype>(
        net_param->num(), net_param->channels(), net_param->height(),
        net_param->width())));
...
void AdaDeltaSolver<Dtype>::SnapshotSolverState(SolverState* state) {
  ...
  for (int i = 0; i < this->update_history_.size(); ++i) {
    // Add history
    BlobProto* history_blob = state->add_history();
    this->update_history_[i]->ToProto(history_blob);
  }
}

template <typename Dtype>
void AdaDeltaSolver<Dtype>::RestoreSolverState(const SolverState& state) {
  CHECK_EQ(state.history_size(), 2 * this->history_.size())
      << "Incorrect length of gradient history blobs.";
  CHECK_EQ(state.history_size(), 2 * this->update_history_.size())
      << "Incorrect length of update history blobs.";
  LOG(INFO) << "SGDSolver: restoring history";
  for (int i = 0; i < this->history_.size(); ++i) {
    this->history_[i]->FromProto(state.history(i));
  }
  for (int i = 0; i < this->update_history_.size(); ++i) {
    this->update_history_[i]->FromProto(state.history(i + this->history_.size()));
  }
}
...
void AdaDeltaSolver<Dtype>::ComputeUpdateValue() {
  ...
  case Caffe::CPU:
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      caffe_cpu_axpby(net_params[param_id]->count(), Dtype(1) + delta,
                this->history_[param_id]->cpu_data(), Dtype(0),
                this->update_[param_id]->mutable_cpu_data());

      // divide history of updates by history of gradients
      caffe_div(net_params[param_id]->count(),
                this->update_history_[param_id]->cpu_data(),
                this->update_[param_id]->cpu_data(),
                this->update_[param_id]->mutable_cpu_data());
      ...
      caffe_cpu_axpby(net_params[param_id]->count(), Dtype(1) - momentum,
          this->update_[param_id]->cpu_data(), momentum,
          this->update_history_[param_id]->mutable_cpu_data());
    }
    break;
  case Caffe::GPU:
#ifndef CPU_ONLY
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      caffe_gpu_add(net_params[param_id]->count(),
                    this->update_[param_id]->gpu_data(),
                    this->update_history_[param_id]->gpu_data(),
                    this->update_[param_id]->mutable_gpu_data());
      ...
      caffe_gpu_axpby(net_params[param_id]->count(), Dtype(1) - momentum,
          this->update_[param_id]->gpu_data(), momentum,
          this->update_history_[param_id]->mutable_gpu_data());
    }
#else
    NO_GPU;
#endif
